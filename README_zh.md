...
- **核心库（有维护示例）**：`canvas2d`、`gsap`、`animejs`、`d3`、`three`、`pixi.js`、`lottie-web`、`matter-js`
- **扩展库（有独立示例）**：`fabric`、`konva`、`paper`、`p5`、`tsParticles`、`cannon-es`、`roughjs`、`@tweenjs/tween.js`、`katex`、`iconify`、`opentype.js`、`split-type`
- **实验库（代码支持，暂无维护示例）**：`postprocessing`、`simplex-noise`、`mathjs`

维护中的示例都要求「库真的改变画面」。只写在依赖里、但没有可见效果的库，不算真正用到。
...
| [`library-timeline`](examples/library-timeline/README.zh.md) | 多库时间线，每段都有可见效果。 | `gsap`、`d3`、`THREE`、`Matter`、`rough`、`SplitType` |

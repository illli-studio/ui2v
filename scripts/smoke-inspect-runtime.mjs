...
const runtimeExample = resolve(root, 'examples/runtime-storyboard/animation.json');
const legacyExample = resolve(root, 'examples/basic-smoke/animation.json');

const jsonResult = run(['inspect-runtime', runtimeExample, '--json', '--time', '1', '--time', '4']);
const payload = JSON.parse(jsonResult.stdout);

if (!payload.valid || payload.inspection.id !== 'runtime-storyboard') {
...
const fullJsonResult = run(['inspect-runtime', runtimeExample, '--json', '--full', '--time', '1']);

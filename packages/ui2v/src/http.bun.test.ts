...
function createBunClient(options?: {
  ...
  const mkdtempImpl = vi.fn(async () => options?.mkdtempValue ?? "/tmp/ui2v-test");
...
describe("bun http client", () => {
  ...
  it("supports fetchText and downloadZip via curl", async () => {
    const { client, spawnImpl, readFileImpl, rmImpl } = createBunClient({
      ...
      mkdtempValue: "/tmp/ui2v-download-abc",
    ...
    expect(rmImpl).toHaveBeenCalledWith("/tmp/ui2v-download-abc", {
      recursive: true,
      force: true,
    });
    expect(spawnImpl).toHaveBeenCalledTimes(3);
  ...
  it("posts multipart form data via curl and cleans up temp files", async () => {
    const { client, spawnImpl, mkdirImpl, writeFileImpl, rmImpl } = createBunClient({
      spawnImpl: () => ({ status: 0, stdout: '{"ok":true}\n200', stderr: "" }),
      mkdtempValue: "/tmp/ui2v-upload-abc",
    });
    ...
    const mkdirCall = mkdirImpl.mock.calls[0] as unknown[] | undefined;
    expect(mkdirCall).toBeDefined();
    const mkdirPath = mkdirCall?.[0];
    expect(typeof mkdirPath).toBe("string");
    expect(normalizePath(String(mkdirPath))).toBe("/tmp/ui2v-upload-abc/dist");
    expect(mkdirCall?.[1]).toEqual({ recursive: true });
    expect(writeFileImpl).toHaveBeenCalled();
    expect(rmImpl).toHaveBeenCalledWith("/tmp/ui2v-upload-abc", {
      recursive: true,
      force: true,
    });
    const spawnCall = spawnImpl.mock.calls[0];
    expect(spawnCall).toBeDefined();
    const args = (spawnCall?.[1] ?? []) as string[];
    expect(args).toContain("-F");
    expect(args.some((arg) => arg.includes("name=demo"))).toBe(true);
    expect(
      args.some((arg) => normalizePath(arg).includes("file=@/tmp/ui2v-upload-abc/dist/demo.txt")),
    ).toBe(true);
  });
});

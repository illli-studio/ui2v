...
import {
  type Lockfile,
  LockfileSchema,
  MOTION_PACKAGE_FILE_EXTENSION_SET,
  parseArk,
  TEXT_FILE_EXTENSION_SET,
} from "./schema/index.js";

const DOT_DIR = ".ui2v";
const LEGACY_DOT_DIRS = [".clawhub", ".clawdhub"] as const;
const DOT_IGNORE = ".ui2vignore";
const LEGACY_DOT_IGNORES = [".clawhubignore", ".clawdhubignore"] as const;
...
export async function listTextFiles(root: string) {
  return listPackageFiles(root, TEXT_FILE_EXTENSION_SET);
}

export async function listMotionPackageFiles(root: string) {
  return listPackageFiles(root, MOTION_PACKAGE_FILE_EXTENSION_SET);
}

async function listPackageFiles(root: string, allowedExtensions: Set<string>) {
  ...
  for (const ignoreName of LEGACY_DOT_IGNORES) {
    await addIgnoreFile(ig, join(absRoot, ignoreName));
  }
  ...
  await walk(absRoot, async (absPath) => {
    ...
    const contentType =
      mime.getType(relPath) ??
      (TEXT_FILE_EXTENSION_SET.has(ext) ? "text/plain" : "application/octet-stream");
...
export async function readLockfile(workdir: string): Promise<Lockfile> {
  const paths = [
    ...
    ...LEGACY_DOT_DIRS.map((dir) => join(workdir, dir, "lock.json")),
  ];
...
export async function readSkillOrigin(skillFolder: string): Promise<SkillOrigin | null> {
  const paths = [
    ...
    ...LEGACY_DOT_DIRS.map((dir) => join(skillFolder, dir, "origin.json")),
  ];
  for (const path of paths) {
    try {
      const raw = await readFile(path, "utf8");

...
function normalizeSlug(slugArg: string, label = "Motion slug") {
...
export async function cmdRenameSkill(
  ...
  try {
    const result = await apiRequest(
      ...
      {
        method: "POST",
        path: `${ApiRoutes.motions}/${encodeURIComponent(slug)}/rename`,
        token,
        body: JSON.stringify({ newSlug }),
      },
      ApiV1SkillRenameResponseSchema,
    );
    const parsed = parseArk(ApiV1SkillRenameResponseSchema, result, "Rename motion response");
...
export async function cmdMergeSkill(
  ...
  try {
    const result = await apiRequest(
      ...
      {
        method: "POST",
        path: `${ApiRoutes.motions}/${encodeURIComponent(sourceSlug)}/merge`,
        token,
        body: JSON.stringify({ targetSlug }),
      },
      ApiV1SkillMergeResponseSchema,
    );
    const parsed = parseArk(ApiV1SkillMergeResponseSchema, result, "Merge motion response");

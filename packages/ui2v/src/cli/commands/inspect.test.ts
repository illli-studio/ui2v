...
describe("cmdInspect", () => {
  it("fetches latest version files when --files is set", async () => {
    ...
    expect(firstArgs?.path).toBe(`${ApiRoutes.motions}/${encodeURIComponent("demo")}`);
    expect(secondArgs?.path).toBe(
      `${ApiRoutes.motions}/${encodeURIComponent("demo")}/versions/${encodeURIComponent("1.2.3")}`,
    );
  });
  ...
  it("uses tag param when fetching a file", async () => {
    ...
    expect(url.pathname).toBe("/api/v1/motions/demo/file");

...
import * as skillStore from "../../motions.js";
...
const commandSkillsModuleSpecifier = "./motions.js?command-motions-test" as string;
...
const {
  ...
  cmdUpdate,
  formatExploreLine,
} = (await import(commandSkillsModuleSpecifier)) as typeof import("./motions.js");
const {
  extractZipToDir,
  hashSkillFiles,
  listTextFiles,
...
const expectedDemoDir = join("/work", "motions", "demo");
...
beforeEach(() => {
  ...
  readLockfileMock.mockResolvedValue({ version: 1, motions: {} });
...
describe("cmdExplore", () => {
  ...
  it("clamps limit and handles empty results", async () => {
    ...
    expect(mockLog).toHaveBeenCalledWith("No motions found.");
  });
...
describe("cmdUpdate", () => {
  it("uses path-based skill lookup when no local fingerprint is available", async () => {
    ...
    vi.mocked(readLockfile).mockResolvedValue({
      version: 1,
      motions: { demo: { version: "0.1.0", installedAt: 123 } },
    });
    ...
    const [, args] = mockApiRequest.mock.calls[0] ?? [];
    expect(args?.path).toBe(`${ApiRoutes.motions}/${encodeURIComponent("demo")}`);
...
describe("cmdInstall", () => {
  it("passes optional auth token to API + download requests", async () => {
    ...
    mockDownloadZip.mockResolvedValue(new Uint8Array([1, 2, 3]));
    vi.mocked(readLockfile).mockResolvedValue({ version: 1, motions: {} });
    vi.mocked(writeLockfile).mockResolvedValue();
    vi.mocked(writeSkillOrigin).mockResolvedValue();
    vi.mocked(extractZipToDir).mockResolvedValue();
    vi.mocked(stat).mockRejectedValue(new Error("missing"));
  ...
  it("does not rm local directory when requested version lookup fails (--force)", async () => {
    ...
    expect(mockApiRequest).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        path: `${ApiRoutes.motions}/${encodeURIComponent("demo")}/versions/${encodeURIComponent("9.9.9")}`,
      }),
      expect.anything(),
    );
  });
  ...
  it("validates requested version before rm when all checks pass (--force)", async () => {
    ...
    expect(rm).toHaveBeenCalledWith(expectedDemoDir, { recursive: true, force: true });
...
describe("cmdUninstall", () => {
  it("requires --yes when input is disabled", async () => {
    vi.mocked(readLockfile).mockResolvedValue({
      version: 1,
      motions: { demo: { version: "1.0.0", installedAt: 123 } },
    });
  ...
  it("removes skill and updates lockfile keeping other motions", async () => {
    vi.mocked(readLockfile).mockResolvedValue({
      version: 1,
      motions: {
    ...
    expect(writeLockfile).toHaveBeenCalledWith("/work", {
      version: 1,
      motions: { other: { version: "2.0.0", installedAt: 456 } },
    });
  });

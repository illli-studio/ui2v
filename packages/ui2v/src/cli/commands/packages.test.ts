...
async function makeTmpWorkdir() {
  return await mkdtemp(join(tmpdir(), "ui2v-package-"));
}
...
describe("package commands", () => {
  ...
  it("supports motion family package browse requests", async () => {
    httpMocks.apiRequest.mockResolvedValueOnce({
      items: [],
      nextCursor: null,
    });
  ...
  it("rejects code-plugin publish when required OpenClaw compatibility metadata is missing", async () => {
    ...
    try {
      ...
      await expect(
        ...
        "openclaw.compat.pluginApi is required for external code plugins published to UI2V.",
      );
      expect(httpMocks.apiRequestForm).not.toHaveBeenCalled();
    } finally {
      await rm(workdir, { recursive: true, force: true });
  ...
  it("respects package ignore rules and built-in ignored directories", async () => {
    ...
    try {
      ...
      await writeFile(join(folder, ".ui2vignore"), "ignored.txt\n", "utf8");
  ...
  it("gets trusted publisher config for a package", async () => {
    httpMocks.apiRequest.mockResolvedValueOnce({
      trustedPublisher: {
        ...
        repositoryOwnerId: "2",
        workflowFilename: "plugin-ui2v-release.yml",
        environment: "ui2v-release",
      },
    });
    ...
    expect(mockLog).toHaveBeenCalledWith("Workflow: plugin-ui2v-release.yml");
    expect(mockLog).toHaveBeenCalledWith("Environment: ui2v-release");
  });

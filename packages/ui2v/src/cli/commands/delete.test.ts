...
const { cmdDeleteSkill, cmdHideSkill, cmdUndeleteSkill, cmdUnhideSkill } = await import("./delete.js");
...
describe("delete/undelete", () => {
  ...
  it("calls delete endpoint with --yes", async () => {
    ...
    expect(httpMocks.apiRequest).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({ method: "DELETE", path: "/api/v1/motions/demo" }),
      expect.anything(),
    );
  });
  ...
  it("calls undelete endpoint with --yes", async () => {
    ...
    expect(httpMocks.apiRequest).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({ method: "POST", path: "/api/v1/motions/demo/undelete" }),
      expect.anything(),
    );
  });

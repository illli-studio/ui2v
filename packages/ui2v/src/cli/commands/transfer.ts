...
function normalizeSlug(slugArg: string) {
  ...
  if (!slug) fail("Motion slug required");
  return slug;
}
...
export async function cmdTransferRequest(
  ...
  try {
    const result = await apiRequest(
      ...
      {
        method: "POST",
        path: `${ApiRoutes.motions}/${encodeURIComponent(slug)}/transfer`,
        token,
        body: JSON.stringify({
...
async function runTransferDecision(
  ...
  try {
    const result = await apiRequest(
      ...
      {
        method: "POST",
        path: `${ApiRoutes.motions}/${encodeURIComponent(slug)}/transfer/${spec.action}`,
        token,
      },
      ApiV1TransferDecisionResponseSchema,
    );

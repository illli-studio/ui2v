...
export async function cmdBanUser(
  ...
  if (!options.yes) {
    ...
    const ok = await promptConfirm(
      `Ban ${resolved.label}? (requires moderator/admin; deletes owned motions)`,
    );
...
function formatUserLabel(user: UserSearchItem) {
  ...
  const label = name ? `${handle} 閳�?${name}` : handle;
  return `${label}${role} 璺� ${user.userId}`;
}
...
function formatDeletedSkills(count: number) {
  if (!Number.isFinite(count)) return "deleted motions unknown";
  if (count === 1) return "deleted 1 motion";
  return `deleted ${count} motions`;
}

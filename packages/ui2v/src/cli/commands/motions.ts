...
export async function cmdInstall(
  ...
  try {
    // Fetch motion metadata including moderation status
    const skillMeta = await apiRequest(
      registry,
      { method: "GET", path: `${ApiRoutes.motions}/${encodeURIComponent(trimmed)}`, token },
      ApiV1SkillResponseSchema,
    );
    ...
    if (skillMeta.moderation?.isMalwareBlocked) {
      ...
      fail("This motion has been flagged as malware and cannot be installed.");
    }
    ...
    if (skillMeta.moderation?.isSuspicious && !force) {
      ...
      console.log(
        `\n⚠️  Warning: "${trimmed}" is flagged as suspicious by VirusTotal Code Insight.\n` +
          "   This motion may contain risky patterns (crypto keys, external APIs, eval, etc.)\n" +
          "   Review the motion code before use.\n",
      );
      if (isInteractive()) {
        ...
        fail("Use --force to install suspicious motions in non-interactive mode");
      }
    }
    ...
    if (versionFlag) {
      await apiRequest(
        ...
        {
          method: "GET",
          path: `${ApiRoutes.motions}/${encodeURIComponent(trimmed)}/versions/${encodeURIComponent(
            resolvedVersion,
          )}`,
          token,
        },
...
export async function cmdUpdate(
  ...
  const registry = await getRegistry(opts, { cache: true });
  const lock = await readLockfile(opts.workdir);
  const slugs = slug ? [slug] : Object.keys(lock.motions).filter(isSafeSkillSlug);
  if (slugs.length === 0) {
    console.log("No installed motions.");
    return;
  }
  ...
  for (const entry of slugs) {
    ...
    try {
      ...
      // Always fetch motion metadata to check moderation status
      const skillMeta = await apiRequest(
        registry,
        { method: "GET", path: `${ApiRoutes.motions}/${encodeURIComponent(entry)}`, token },
        ApiV1SkillResponseSchema,
      );
      ...
      if (skillMeta.moderation?.isMalwareBlocked) {
        ...
        console.log("   This motion has been flagged as malware and cannot be updated.");
        continue;
      }
      ...
      if (skillMeta.moderation?.isSuspicious && !options.force) {
        ...
        if (allowPrompt) {
          ...
          console.log(`${entry}: skipped (use --force to update suspicious motions)`);
          continue;
        }
      }
      ...
      if (matched && lock.motions[entry]?.version !== matched) {
        lock.motions[entry] = {
          version: matched,
          installedAt: lock.motions[entry]?.installedAt ?? Date.now(),
        };
      }
      ...
      lock.motions[entry] = { version: targetVersion, installedAt: Date.now() };
...
export async function cmdList(opts: GlobalOpts) {
  const lock = await readLockfile(opts.workdir);
  const entries = Object.entries(lock.motions);
  if (entries.length === 0) {
    console.log("No installed motions.");
    return;
  }
...
export async function cmdExplore(
  ...
  const token = await getOptionalAuthToken();
  const registry = await getRegistry(opts, { cache: true });
  const spinner = createSpinner("Fetching latest motions");
  try {
    const url = registryUrl(ApiRoutes.motions, registry);

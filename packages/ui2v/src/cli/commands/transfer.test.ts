...
describe("transfer commands", () => {
  ...
  it("request calls transfer endpoint", async () => {
    httpMocks.apiRequest.mockResolvedValueOnce({
      ok: true,
      transferId: "motionOwnershipTransfers:1",
      toUserHandle: "alice",
    ...
    expect(httpMocks.apiRequest).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: "POST",
        path: "/api/v1/motions/demo/transfer",
      }),
      expect.anything(),
    );
  ...
  it("accept/reject/cancel call action endpoints", async () => {
    ...
    expect(httpMocks.apiRequest).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({ method: "POST", path: "/api/v1/motions/demo/transfer/accept" }),
      expect.anything(),
    );
    expect(httpMocks.apiRequest).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({ method: "POST", path: "/api/v1/motions/demo/transfer/reject" }),
      expect.anything(),
    );
    expect(httpMocks.apiRequest).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({ method: "POST", path: "/api/v1/motions/demo/transfer/cancel" }),
      expect.anything(),
    );
  });
});

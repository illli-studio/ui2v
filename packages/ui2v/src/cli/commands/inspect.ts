...
export async function cmdInspect(opts: GlobalOpts, slug: string, options: InspectOptions = {}) {
  ...
  const spinner = createSpinner("Fetching motion");
  try {
    ...
    if (options.versions) {
      ...
      const url = registryUrl(
        `${ApiRoutes.motions}/${encodeURIComponent(trimmed)}/versions`,
        registry,
      );
    ...
    if (options.file) {
      const url = registryUrl(`${ApiRoutes.motions}/${encodeURIComponent(trimmed)}/file`, registry);
...
function printMotionSummary(result: {
  skill: {
    slug: string;
    displayName: string;
    summary?: string | null;
  ...
  console.log(
    `License: ${result.versionLicense ?? result.latestVersion?.license ?? PLATFORM_MOTION_LICENSE} (${PLATFORM_SKILL_LICENSE_SUMMARY})`,
  );
  const tags = normalizeTags(skill.tags);

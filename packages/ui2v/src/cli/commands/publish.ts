...
import { ApiRoutes, ApiV1PublishResponseSchema, isRegistryItemPath } from "../../schema/index.js";
import { listMotionPackageFiles } from "../../motions.js";
...
export async function cmdPublish(
  ...
  if (await looksLikePluginFolder(folder)) {
    fail('This looks like a plugin. Use "ui2v package publish <source>" instead.');
  }
  ...
  try {
    const filesOnDisk = await listMotionPackageFiles(folder);
    if (filesOnDisk.length === 0) fail("No files found");
    if (!filesOnDisk.some((file) => isRegistryItemPath(file.relPath))) {
      fail("registry-item.json required");
    }
    if (
      !filesOnDisk.some((file) => {
        const lower = file.relPath.replace(/\\/g, "/").toLowerCase();
        return lower === "index.html" || lower.endsWith(".html");
      })
    ) {
      fail("Entry composition HTML required (index.html or *.html)");
    }
    ...
    const result = await apiRequestForm(
      registry,
      { method: "POST", path: ApiRoutes.motions, token, form },

...
import { hashSkillZip } from "../../motions.js";
import { getRegistry } from "../registry.js";
import { findSkillFolders, type SkillFolder } from "../scanMotions.js";
...
export async function reportTelemetryIfEnabled(params: {
  ...
  const roots = params.scan.roots.map((root) => ({
    ...
    motions: (params.scan.skillsByRoot[root] ?? []).map((skill) => ({
      slug: skill.slug,
...
function isTelemetryDisabled() {
  const raw =
    process.env.UI2V_DISABLE_TELEMETRY ??
    process.env.CLAWHUB_DISABLE_TELEMETRY ??
    process.env.CLAWDHUB_DISABLE_TELEMETRY;
  if (!raw) return false;
...
export async function checkRegistrySyncState(
  ...
  const meta = await apiRequest(
    registry,
    { method: "GET", path: `${ApiRoutes.motions}/${encodeURIComponent(skill.slug)}`, token },
    ApiV1SkillResponseSchema,
  ).catch(() => null);
...
export async function scanRootsWithLabels(roots: string[], labels?: Record<string, string>) {
  ...
  return {
    roots: uniqueRoots,
    skillsByRoot,
    motions: Array.from(byFolder.values()),
    rootsWithSkills,
    rootLabels,
  };
}
...
export function mergeScan(
  ...
  const byFolder = new Map<string, SkillFolder>();
  for (const entry of [...left.motions, ...right.motions]) {
    byFolder.set(entry.folder, entry);
  }
  const motions = Array.from(byFolder.values());
  const rootsWithSkills = mergedRoots.filter((root) => (skillsByRoot[root]?.length ?? 0) > 0);
  return { roots: mergedRoots, skillsByRoot, motions, rootsWithSkills, rootLabels };
}
...
export async function selectToUpload(
  ...
  const picked = await multiselect({
    message: "Select motions to upload",
    options: choices,
...
export function dedupeSkillsBySlug(motions: SkillFolder[]) {
  const bySlug = new Map<string, SkillFolder[]>();
  for (const skill of motions) {
  ...
  return { motions: unique, duplicates };
}

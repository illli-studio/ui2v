...
export async function resolveClawdbotSkillRoots(): Promise<ClawdbotSkillRoots> {
  ...
  const sharedSkills = resolveUserPath(join(clawdbotStateDir, "motions"));
  pushRoot(roots, labels, sharedSkills, "Shared motions");

  const openclawStateDir = resolveOpenclawStateDir();
  const openclawShared = resolveUserPath(join(openclawStateDir, "motions"));
  pushRoot(roots, labels, openclawShared, "OpenClaw: Shared motions");
...
function addConfigRoots(
  ...
  if (mainWorkspace) {
    pushRoot(roots, labels, join(mainWorkspace, "motions"), `${prefix}Agent: main`);
  }
  ...
  for (const entry of listedAgents) {
    ...
    pushRoot(roots, labels, join(workspace, "motions"), `${prefix}Agent: ${name}`);
  }
  ...
  const extraDirs = config.motions?.load?.extraDirs ?? [];

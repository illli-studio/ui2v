...
const { writeGlobalConfig } = (await import(configModuleSpecifier)) as typeof import("./config.js");

const originalPlatform = process.platform;
const testConfigPath = "/tmp/ui2v-config-test/config.json";
const expectedConfigPath = resolve(testConfigPath);
const expectedConfigDir = dirname(expectedConfigPath);
...
beforeEach(() => {
  envStubs.stub("UI2V_CONFIG_PATH", testConfigPath);
...
describe("writeGlobalConfig", () => {
  it("writes config with restricted modes", async () => {
    ...
    expect(fsMocks.mkdir).toHaveBeenCalledWith(expectedConfigDir, {
      recursive: true,
      mode: 0o700,
    });
    ...
    expect(fsMocks.chmod).toHaveBeenCalledWith(expectedConfigPath, 0o600);
  });

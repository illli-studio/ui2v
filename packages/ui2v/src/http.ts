...
const CURL_META_MARKER = "__UI2V_CURL_META__";
...
async function fetchJsonFormViaCurl(
  ...
  const tempDir = await deps.mkdtempImpl(join(deps.tmpdirPath, "ui2v-upload-"));
  try {
...
async function fetchBinaryViaCurl(
  ...
  const tempDir = await deps.mkdtempImpl(join(deps.tmpdirPath, "ui2v-download-"));

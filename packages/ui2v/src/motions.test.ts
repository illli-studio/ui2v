...
import type { SkillOrigin } from "./motions";
import {
...
describe("motions", () => {
  it("extracts zip into directory and skips traversal", async () => {
    const parent = await mkdtemp(join(tmpdir(), "ui2v-zip-"));
  ...
  it("writes and reads lockfile", async () => {
    const workdir = await mkdtemp(join(tmpdir(), "ui2v-work-"));
    await writeLockfile(workdir, {
      version: 1,
      motions: { demo: { version: "1.0.0", installedAt: 1 } },
    });
    const read = await readLockfile(workdir);
    expect(read.motions.demo?.version).toBe("1.0.0");
  });

  it("returns empty lockfile on invalid json", async () => {
    const workdir = await mkdtemp(join(tmpdir(), "ui2v-work-bad-"));
    await mkdir(join(workdir, ".ui2v"), { recursive: true });
    await writeFile(join(workdir, ".ui2v", "lock.json"), "{", "utf8");
    const read = await readLockfile(workdir);
    expect(read).toEqual({ version: 1, motions: {} });
  });

  it("returns empty lockfile on schema mismatch", async () => {
    const workdir = await mkdtemp(join(tmpdir(), "ui2v-work-schema-"));
    await mkdir(join(workdir, ".ui2v"), { recursive: true });
    await writeFile(
      join(workdir, ".ui2v", "lock.json"),
      JSON.stringify({ version: 1, motions: "nope" }),
      "utf8",
    );
    const read = await readLockfile(workdir);
    expect(read).toEqual({ version: 1, motions: {} });
  ...
  it("skips dotfiles and node_modules when listing text files", async () => {
    const workdir = await mkdtemp(join(tmpdir(), "ui2v-files-"));
  ...
  it("respects .gitignore and .ui2vignore", async () => {
    const workdir = await mkdtemp(join(tmpdir(), "ui2v-ignore-"));
    await writeFile(join(workdir, ".gitignore"), "ignored.md\n", "utf8");
    await writeFile(join(workdir, ".ui2vignore"), "private.md\n", "utf8");
    await writeFile(join(workdir, "SKILL.md"), "hi", "utf8");
  ...
  it("falls back to text/plain for unknown text extensions", async () => {
    const workdir = await mkdtemp(join(tmpdir(), "ui2v-env-"));
    await writeFile(join(workdir, "SKILL.md"), "hi", "utf8");
  ...
  it("returns null for invalid skill origin metadata", async () => {
    const workdir = await mkdtemp(join(tmpdir(), "ui2v-origin-"));
    expect(await readSkillOrigin(workdir)).toBeNull();

    await mkdir(join(workdir, ".ui2v"), { recursive: true });
    await writeFile(
      join(workdir, ".ui2v", "origin.json"),
